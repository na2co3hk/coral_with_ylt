...
#ifndef CORAL_WITH_YLT_HTTP_REQUEST_HPP
#define CORAL_WITH_YLT_HTTP_REQUEST_HPP
...
private:
    ...
    void parseRequestLine(const std::string& line) {
        ...
        if (subMatch.size() < 3) {
            err_ = 1;
            coral::log.Error("RequestLine parse error");
            return;
        }
        else {
            method_ = subMatch[0];
    ...
    void parseHeader(const std::string& line) {
        ...
        if (subMatch.size() < 2) {
            err_ = 1;
            coral::log.Info("Headers parse error");
            return;
        }
        else {
...
#endif //CORAL_WITH_YLT_HTTP_REQUEST_HPP
